...
# Generate a sorted list with random numbers ensuring 24 is included
numbers = sorted(random.sample(range(1, 100), 19) + [24])
...
def on_draw():
    ...
    for i, number in enumerate(numbers):
        ...
        if left <= i <= right and not search_complete:
            color = (255, 0, 255)  # Magenta for the current search interval
        elif i == mid and not search_complete:
            color = (0, 0, 255)  # Blue for the middle element
        elif found and i == mid:
            color = (0, 255, 0)  # Green if 24 is found
        else:
